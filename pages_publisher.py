...
def publish_to_pages(html_content: str, article_count: int) -> str | None:
    """
    Clone la branche gh-pages, ajoute le rapport, met à jour index.html, pousse.
    Retourne l'URL publique ou None si échec.
    """
    token = os.environ.get("GITHUB_TOKEN", "")
    repo  = os.environ.get("GITHUB_REPOSITORY", "bigmoletos/windows-dev-toolkit")
    ...
    try:
        ...
        # Cloner uniquement la branche gh-pages (ou créer si absente)
        ...
        if result.returncode != 0:
            # Branche gh-pages inexistante — init orphan
        ...
        # Écrire le rapport du jour
        (Path(tmpdir) / filename).write_text(html_content, encoding="utf-8")

        # Reconstruire index.html avec tous les rapports existants
        reports = sorted(
            [f.name for f in Path(tmpdir).glob("revue_ia_*.html")],
            reverse=True
        )
        ...
        subprocess.run(
            ["git", "-C", tmpdir, "commit", "-m", f"revue IA {today} ({article_count} articles)"],
            check=True
        )
        subprocess.run(
...
def _write_index(tmpdir: str, reports: list[str]):
    """Génère index.html listant tous les rapports."""
    items = ""
    for r in reports:
        d = r.replace("revue_ia_", "").replace(".html", "")
...
<body>
<h1>Revue de Presse IA</h1>
<p>Mise à jour quotidienne — lundi au vendredi à 7h30</p>
<ul>
{items}
</ul>
</body>
